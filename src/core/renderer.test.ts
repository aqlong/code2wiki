...
describe("renderUseCase always-present sections", () => {
  ...
  it("emits a placeholder bullet in Source links when source_files is empty (no blank <details>)", () => {
    // The UseCase schema permits an empty source_files array, and
    // renderUseCase is public (the dashboard preview lib + replay call it with
    // hand-built use cases, not only the extractor, which always supplies one
    // entry). Without the placeholder the always-present Source links section
    // renders a <details> block with a blank body, an auditor-facing empty
    // "Implementation files" dropdown. Pin the graceful-degradation contract.
    const md = renderUseCase(baseUseCase({ source_files: [] }));
    expect(md).toContain("## Source links");
    expect(md).toContain("- (no source files recorded)");
    // The details body must not be blank: the placeholder sits between the
    // <summary> and the closing </details>.
    const detailsBody = md.slice(
      md.indexOf("</summary>") + "</summary>".length,
      md.indexOf("</details>"),
    );
    expect(detailsBody).toContain("- (no source files recorded)");
    expect(detailsBody.trim()).not.toBe("");
  });
});
...
describe("renderUseCase managed fence", () => {
  it("emits the managed fence with the code2wiki_id and the confidence + reason inside", () => {
    ...
    expect(md).toContain(
      "*Generated by [code2wiki](https://github.com/craftandship/code2wiki) from commit `deadbee` on 2026-01-02T03:04:05Z.*",
    );
    // No inline HTML in the footer: the Confluence storage renderer
    // escapes raw tags, so a <time> wrapper would publish as literal
    // markup text (regression shipped 2026-07-02, reverted same day).
