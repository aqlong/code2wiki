...
function bannerHtml(banner: BannerInputs): string {
  return (
    ...
    `Last synced: ${escapeHtml(banner.lastSyncedIso)}. ` +
    `<a href="${escapeHtml(banner.sourceLink)}">View source</a>.`
  );
}

function renderIndex(entries: Array<{ slug: string; title: string }>): string {
  const items = entries
    .map(
      (e) =>
        `<li>
...
<p class="preamble">Browsable preview of what each generated page would look like once published. No network calls; nothing was published. Generated ${new Date().toISOString()}.</p>
<ul>${items}</ul>
</body></html>`;
}
